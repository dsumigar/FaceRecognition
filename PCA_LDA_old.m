...
outputFolder_train = 'C:\Users\sumig\OneDrive\Documents\MATLAB\EC520 HW\Face ID Project\PreprocecessedTrain\';    % Initialize folder to hold pre-processed training set of face images
...
percentage_best = find(percentage_correct > 95);
...
% First, use PCA to reduce the dimension to N - c (==90 - 15)
PCA_on_LDA = LDA_images_train*LDA_images_train';
[V_PCA_LDA, ~] = eig(PCA_on_LDA);

reduced_dim = m - length(Subjects);

LDA_basis_reduced = V_PCA_LDA(:, end - reduced_dim + 1:end);    % Want to take the first m - k eigenvectors (reduce the basis)
LDA_basis_reduced = LDA_images_train' * LDA_basis_reduced;      % Project the the training images onto the reduced basis
PCA_LDA_train = LDA_images_train * LDA_basis_reduced;           % Compute projection matrix 
...
for i = 1 : length(Subjects)
    ...
    mean_PCA_LDA_i(i,:) = mean(PCA_LDA_train(lTrain==i,:),1);   % Compute u_i
    temp_S_b = mean_PCA_LDA_i(i,:) - mean_PCA_LDA_train;        % Determine the difference between the mean of each class and the overall mean
...
[V_scatter_reduced_dim,~] = eig(S_b,S_w);                                           % Find eigenvectors of scatter matrices (forming the basis of Wfld)

% Repeat the previous computations in the reduced dimension:
Wfld = V_scatter_reduced_dim(:, end - reduced_dim_2 + 1:end);                       % Further reduction of basis to c - 1 -> Wfld
Wfld_Wpca = LDA_basis_reduced * Wfld;                                               % Product of Wfld and Wpca = Wopt        
u = LDA_images_train * Wfld_Wpca;                                                   % u = Wfld*Wpca*x (Projection onto data to maximize class separation)           

% mean_LDA = mean(u,1);
% 
% for i = 1 : length(Subjects)
%     mean_LDA_i(i,:) = mean(u(lTrain==i,:),1);
% end
